...
class CustomPasswordValidator:
    def validate(self, password, user=None):
        ...
        if not any(char.isupper() for char in password):
            errors.append('Password must contain at least 1 upper letter.')
        if user == password:
    ...
    def get_help_text(self):
        return "Yor password must contain at least 1 digit, 1 letter, 1 special character, 1 upper letter."
